...
export default function Index() {
  ...
  const currentLocationIndex = useAppSelector(
    (state) => state.locations.currentLocationIndex,
  );
  ...
  function saveTask() {
    if (task === "" || currentLocationIndex === -1) return;
  ...
  useEffect(() => {
    Location.startGeofencingAsync(
      "Geofencing",
      locations.map((l) => ({ identifier: l.name, ...l })),
    ...
    >
      <Text style={{ marginVertical: 20 }}>
        {currentLocationIndex !== -1
          ? `Currently at ${locations[currentLocationIndex].name}`
      ...
      <View style={{ flex: 1 }}>
        ...
        {currentLocationIndex !== -1 &&
        locations[currentLocationIndex].tasks.length !== 0
          ? locations[currentLocationIndex].tasks.map((task) => (
              <Task
                task={task}
                locationName={locations[currentLocationIndex].name}
              />
