...
function findTask(
  ...
  const taskIndex = locations[locationIndex].tasks.findIndex(
...
export const locationsSlice = createSlice({
  ...
  reducers: {
    addLocation: (state, action: PayloadAction<LocationInterface>) => {
      const [locationIndex] = findLocation(
        state.locations,
        action.payload.name,
      );
      if (locationIndex !== -1) {
    ...
    removeLocation: (state, action: PayloadAction<string>) => {
      const [locationIndex, error] = findLocation(
        state.locations,
        action.payload,
      );
      if (error) {
    ...
    updateLocation: (
      ...
      const [locationIndex, error] = findLocation(
        state.locations,
        action.payload.prevLocation.name,
      );
      if (error) {
        state.error = error;
      } else {
    ...
    addTask: (
      state,
      action: PayloadAction<{ locationName: string; taskName: string }>,
    ) => {
      const [locationIndex, taskIndex] = findTask(
        state.locations,
      ...
      if (locationIndex === -1) {
        ...
        state.locations[locationIndex].tasks.push({
    ...
    toggleTaskCompletion: (
      ...
      state.locations[locationIndex].tasks[taskIndex].isCompleted =
        !state.locations[locationIndex].tasks[taskIndex].isCompleted;
      state.error = null;
    },
    removeTask: (
      ...
      const [locationIndex, taskIndex, error] = findTask(
        state.locations,
        action.payload.locationName,
        action.payload.taskName,
      );
      if (error) {
        ...
        state.locations[locationIndex].tasks = state.locations[
          locationIndex
    ...
    updateTask: (
      ...
      const [locationIndex, taskIndex, error] = findTask(
        state.locations,
        action.payload.locationName,
        action.payload.prevTaskName,
      );
      if (error) {
        state.error = error;
      } else {
        // @ts-ignore
        state.locations[locationIndex].tasks[taskIndex] =
